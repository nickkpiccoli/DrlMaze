...
class MazeWorldEnv(gym.Env):
    ...
    def __init__(self, size: int = 5):
        ...
        self.max_repeated_steps = 3
        self.max_repeated_steps = self.max_repeated_steps
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        while len(self.hole_positions) < num_holes:
            ...
            if not any(np.array_equal(pos, ex) for ex in exclude_positions):
                self.hole_positions.append(pos)
    ...
    def step(self, action: ActType):
        ...
        repeated_step = any(np.array_equal(self._agent_pos, pos) for pos in self.previous_positions[-self.max_repeated_steps:])
        truncated = False
        ...
        if terminated:
            ...
            reward = -0.2  # Small positive reward for a valid move

        self.previous_positions.append(self._agent_pos.copy())
...
def train_maze_model():
    ...
    model = PPO("MultiInputPolicy", "MazeWorld-v2", verbose=1).learn(10000)
    model.save("ppo_maze_world_with_dyn_holes_repstep")
    print("Model trained!")

#train_maze_model()


model = PPO.load("ppo_maze_world_with_dyn_holes_repstep")
